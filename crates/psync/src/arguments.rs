  ...
  pub(crate) fn run(self) -> Result {
    ...
    overall.set_style(
      ProgressStyle::with_template(
        "{bar:30.green/white.dim} {bytes:>11}/{total_bytes:<11} {percent:>3}% {binary_bytes_per_sec:>11} {eta:>6}",
      )
      .context(error::Style)?
      .progress_chars("=>-"),
    );
    ...
    for _ in 0..self.threads {
      ...
      bar.set_style(
        ProgressStyle::with_template(
          "{bar:30.cyan/white.dim} {bytes:>11}/{total_bytes:<11} {binary_bytes_per_sec:>11} {wide_msg}",
        )
        .context(error::Style)?
        .progress_chars("=>-"),
      );
